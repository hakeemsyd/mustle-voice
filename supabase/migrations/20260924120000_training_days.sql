alter table training_plan add column training_days smallint[];
...
declare
  ...
  v_prior_training_days smallint[];
  v_training_days smallint[];
begin
  select starts_on, training_days into v_prior_starts_on, v_prior_training_days
  from training_plan where user_id = p_user_id and status = 'active';

  if jsonb_typeof(p_plan->'training_days') = 'array' and jsonb_array_length(p_plan->'training_days') > 0 then
    select array_agg(value::smallint order by value::smallint)
      into v_training_days
      from jsonb_array_elements_text(p_plan->'training_days');
  else
    v_training_days := v_prior_training_days;
  end if;

  update training_plan set status = 'archived' where user_id = p_user_id and status = 'active';

  insert into training_plan (user_id, split, days_per_week, starts_on, training_days)
  values (
    p_user_id,
    p_plan->>'split',
